...
RUN cd /root/ && \ 
  wget https://github.com/Esri/file-geodatabase-api/raw/master/FileGDB_API_1.4/FileGDB_API_1_4-64.tar.gz && \
  tar xvzf FileGDB_API_1_4-64.tar.gz && \
  cd FileGDB_API-64 && \
  cp lib/*.so /usr/lib64
#RUN echo 'include /root/FileGDB_API-64gcc/lib' > /etc/ld.so.conf.d/fgdb.conf

RUN wget https://github.com/OSGeo/gdal/releases/download/v3.3.0/gdal-3.3.0.tar.gz
RUN tar -zvxf gdal-3.3.0.tar.gz
RUN cd gdal-3.3.0 && ./configure --with-proj=/usr/local --with-threads  --with-fgdb=/root/FileGDB_API-64/ --with-libtiff=internal --with-geotiff=internal --with-jpeg=internal --with-gif=internal --with-png=internal --with-libz=internal && export LD_LIBRARY_PATH=/root/FileGDB_API-64gcc/lib:$LD_LIBRARY_PATH && make && make install
